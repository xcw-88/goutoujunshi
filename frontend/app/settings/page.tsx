...
export default function SettingsPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setSettings(updated);
    (event.currentTarget.elements.namedItem("api_key") as HTMLInputElement).value = "";
    setNotice("设置已保存。API Key 仅保留在当前后端进程内存中。")
  }
  ...
  return (
    <div className="page-wrap narrow-page">
      <div className="page-heading"><div><span className="eyebrow">Configuration</span><h1>模型设置</h1><p>连接任意 OpenAI-compatible 接口。配置不会发送到前端以外的服务，除非你主动开始聊天。</p></div></div>
      <section className="surface settings-card">
        {settings ? <form className="stack-form" onSubmit={save}>
          ...
          <label>API Key<input name="api_key" type="password" autoComplete="off" placeholder={settings.api_key_configured ? "留空则保持不变" : "输入 API Key"} /></label>
          <p className="field-help">V1 不把 Key 写入 SQLite 或浏览器存储。重启后端后，如未设置环境变量，需要重新输入。</p>
          <div className="form-actions"><button type="button" className="danger-link" onClick={clearKey} disabled={!settings.api_key_configured}>清除 Key</button><button className="primary">保存设置</button></div>
          {notice && <span className="success-note">{notice}</span>}
        </form> : <div className="empty large">正在读取设置… {notice}</div>}
      </section>
      <section className="privacy-box"><strong>隐私提示</strong><p>聊天内容与截图会发送到你配置的模型服务商。请根据服务商条款决定是否提交敏感资料；本应用本身不提供云同步或账号系统。</p></section>
    </div>
  );
}
