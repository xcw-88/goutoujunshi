...
`GOUTOU_API_KEY`（OpenAI 和其他自定义兼容接口）与 `GOUTOU_GEMINI_API_KEY`（Google Gemini 官方兼容接口）只能作为 Worker Secret 配置，设置页不能写入或清除这些 Key。Gemini Key 只在 Base URL 的主机名**恰好**为 `generativelanguage.googleapis.com` 时使用；其他地址使用通用 Key。聊天正文和图片会发送到用户配置的模型服务商。前端 PWA 只缓存静态壳层，不缓存 API 数据。云端设置中，模型 URL 必须使用 HTTPS。

设置页可快捷选 OpenAI 或 Google Gemini，也可填任意支持 `/chat/completions` 的 OpenAI-compatible HTTPS 接口及其模型 ID。预设只是方便填写，不能保证任意模型都支持本应用发送的聊天、参数和图片格式；处理截图时需选择支持图片输入的模型。Google 免费层对提交内容的数据使用政策与付费层不同，发送私人聊天前请查看服务商条款。切换模型会影响之后的新回复，不会改写已有聊天记录。
...
3. 在 Cloudflare Dashboard 或用 `npx wrangler secret put` 分别设置 `GOUTOU_ACCESS_PASSWORD`（高强度随机访问口令）和 `GOUTOU_SESSION_SECRET`（至少 32 字符的独立随机值）。使用 OpenAI / 自定义兼容接口时设置 `GOUTOU_API_KEY`；使用 Google Gemini 时设置 `GOUTOU_GEMINI_API_KEY`。可同时设置两种 Key，在设置页切换服务商时不必覆盖另一种。不要把值写在 `wrangler.jsonc`、`.dev.vars` 的提交中或命令行参数里。
